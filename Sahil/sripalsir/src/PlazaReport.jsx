...
//       <div className="text-[40px] bg-blue-gray-50 text-center border-2 border-black mb-2">Plaza Entry</div>
...
export default function Reportplaza2() {
  ...
  const [getoa,setgetoa] = useState(false);
  const [formData, setFormData] = useState({
      date_rep:"",
      opening_amt: "",
      adv_from_ho :"",
...
//   fetch(api+'getoa',{
//     method:'POST',
//     body: JSON.stringify({ date: formData.date_rep, plaza: formData.plaza_code  }),
// })
...
// );
  // 
    ...
    // fetch(api+'getoa',{
    //     method:'POST',
    //     body: JSON.stringify({ date: formattedDate,plaza:pid }),
    // })
    // .then((response) => response.json())
    // .then((data) => 
    // handleInputChange("opening_amt", data.oa)
  ...
  fetch(api+'getplazastat',{
    ...
    fetch(api+'get_plaza')
      ...
      if(getoa){
        setgetoa(!getoa);
        // console.log("date: "+formData.test+"plaza: "+pid);
        fetch(api+'getoa',{
          method:'POST',
          body: JSON.stringify({ date: formData.test, plaza: pid  }),
      })
      .then((response) => response.json())
      .then((data) => 
      handleInputChange("opening_amt", data.oa)
...
// console.log(formattedDate);
  fetch(api+'getoa',{
    method:'POST',
    body: JSON.stringify({ date: formattedDate, plaza: value  }),
})
.then((response) => response.json())
.then((data) => 
handleInputChange("opening_amt", data.oa)
  ...
  const renderFormPart = (step) => {
    switch (step) {
      ...
      case 0:
        return(
          <>
          <div className="flex justify-center items-center w-[800px] mt-5">
        ...
        return (
          ...
          <div className="w-full lg:w-6/12 px-4">
            <div className="relative w-full mb-5">
              ...
              value={(formData.opening_amt)+(formData.adv_from_ho)+(formData.total_cash_recievable)+(formData.balaji)+(formData.monthly_pass_amt)-(formData.cash_dep_bank)-(formData.cash_dep_arcpl)-(formData.cash_kpt)-(formData.salary)}
               disabled/>
            </div>
          </div>
          <div className="w-full lg:w-6/12 px-4">
